    ...
    for epoch in range(config.num_epochs):
        ...
        for x_batch, y_batch in batch_train:
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                if total_batch >= 300 and acc_val < 0.8:
                    print('Performance is bad...')
                    return
...
def load_dic(num):
    if not os.path.exists(vocab_dir):
        ...
        # f = open('trainData.txt', 'r', encoding='utf8')
        # documents = f.readlines()
        #
        # # 去掉停用词
        # stoplist = set('for a of the and to in'.split())
...
def build_word_array(word_to_id):
    data={}
    vector_array=[]
    with open("glove_word_vector.txt",'w',encoding='utf8') as o,\
        open("vectors.txt",'r',encoding='utf8') as f:
        ...
        o.write(str(vector_array))
    return word_to_id_copy

class Random_TCNNConfig(object):
    """CNN配置参数"""

    embedding_dim = random.randint(50,300)  # 词向量维度
    seq_length = -1  # 序列长度
    num_classes = 2  # 类别数
    num_filters = random.randint(50,250)  # 卷积核数目
    kernel_size = 5  # 卷积核尺寸
    vocab_size = -1  # 词汇表达小

    hidden_dim = random.randint(100,200)  # 全连接层神经元

    dropout_keep_prob = 0.5  # dropout保留比例
    learning_rate = 1e-3  # 学习率

    batch_size = random.randint(32,256)  # 每批训练大小
    num_epochs = 10  # 总迭代轮次

    print_per_batch = 100  # 每多少轮输出一次结果
    save_per_batch = 10  # 每多少轮存入tensorboard

    Three_filter_open = True    #3中卷积核大小模式

if __name__ == '__main__':
    print('Generating data...')
    data_pack(num_classes)
    print('Configuring CNN model...')
    config = Random_TCNNConfig
    config.num_classes = num_classes
    dictionary,max_len = load_dic(num_classes)
    word_to_id = dictionary.token2id
    if os.path.exists("glove_word_vector.txt")==False:
        word_to_id=build_word_array(word_to_id)
